...
which are unhealthy right now, and which will refuse to upgrade?" means enumerating every tenant,
every subscription and every cluster, then comparing each against what Azure offers in its region. By
hand through the Azure CLI that is one process invocation per tenant per query, run serially, and the
result is terminal scrollback rather than a report. Asking the same about one suspect cluster is a
different problem: there nobody wants a fleet table, they want everything the platform knows about
that cluster, including Azure's own diagnostics. fleetscan answers both, from one binary, at two zoom
levels.
...
exists to prevent. The design must make that impossible structurally, not avoid it carefully. So every
credential enters the process through a child process, and nothing else in fleetscan touches a
credential store. Once per context, fleetscan runs:
...
Units, each with one job. Only `ctxsource` knows cloudctx exists, only `auth` knows a credential ever
came from a CLI, and probes receive an `azcore.TokenCredential` and cannot tell how it was obtained.
That last property is what lets the detector client reuse the credential with no new auth work.
...
`ModeScan`, `ModeDoctor` and `ModeBoth = ModeScan | ModeDoctor`. `Subscription` is a
`ctxsource.Context` plus the subscription ID and display name. `ctxsource.Source` is the one-method
interface returning `[]Context`. `auth.Credential` holds a context name, a `Runner`, a cached token,
an expiry and a mutex, and satisfies `azcore.TokenCredential` simply by having
...
The outer fan-out is over contexts, the inner over subscriptions. `doctor` walks the same path, then
...
`SubscriptionRequestsThrottled` (HTTP 429) across a large fleet, at which point the run is both slow
and incomplete. Every scan-mode source is subscription-scoped for that reason, and the one family of
calls that is inherently per-cluster, the detectors, is therefore a `doctor` feature rather than
something to be rate-limited into `scan`.
...
fleetscan filters server side to `Microsoft.ContainerService/managedClusters` and maps Unavailable and
Degraded to `StatusDegraded`, carrying the reason into `Detail`. Unknown is reported as degraded too,
with its reason stated, because a cluster the platform has lost sight of is not a cluster known to be
healthy.

Advisor recommendations are filterable by category (Cost, HighAvailability, Performance, Security) and
by severity, and are subscription-scoped too. Scan pulls HighAvailability and Security, doctor pulls
all four; Advisor findings annotate and never set an exit code on their own.
`ListKubernetesVersions` is fetched once per distinct location per run and held for that run only.
...
- `apiServerAccessProfile` (private cluster, authorized IP ranges), `disableLocalAccounts`,
  `aadProfile.enableAzureRBAC`, `nodeResourceGroup`.
...
IO, and they predict failures that otherwise surface mid-upgrade with a node pool already in `Failed`.

- **SNAT headroom.** Available ports = outbound IP count x 64,000. Required = node count x
  `allocatedOutboundPorts`. When required exceeds available the cluster cannot scale as configured.
  That inequality is exactly the `InvalidLoadBalancerProfileAllocatedOutboundPorts` error, checkable
...
- So `sku.tier` and `supportPlan` change the answer: an LTS cluster on Premium can be supported where
  ...
  alone, which is why tier and plan are parameters rather than context the caller keeps to itself.
- Upgrades must be sequential through minor versions, except when climbing out of an unsupported
  ...
  prints is a path Azure will accept.

No support window is hardcoded as dates: the offered list comes from Azure per region, and the policy
above is applied to it.
...
(`policy.TokenRequestOptions{Scopes: []string{"https://management.azure.com/.default"}}`). Adding
detectors therefore costs no new authentication work at all.
...
- List response: `value[]` of `{id, name, type, location, properties{metadata{id, name, category,
  description, type}, status{message, statusId}}}`. `statusId` is the numeric severity and is the
...
- Run response: `properties{dataset[]{renderingProperties{title, description, isVisible, type},
  ...
  so any check reading a specific detector must pin that detector's columns and fail loudly if the
  shape changes.
...
- Cache the detector list per `subscription:rg:cluster` for the lifetime of a run, and honor
  `Retry-After` on 429.
...
cluster RBAC and no network path to the Kubernetes API server. The Kubernetes data plane needs all
three, and from a workstation a private cluster is simply unreachable without a jump host or a VPN.
Detectors are what make ARM-only sufficient rather than a compromise: they return the platform's own
analysis of node health, connectivity and storage without the caller ever touching the data plane, so
the deep mode reports Azure's diagnosis instead of fleetscan's guess.

What stays out of reach, listed so nobody is surprised by its absence: node conditions and kubelet
pressure detail, Pending or FailedScheduling pods, OOMKills, Pod Disruption Budgets blocking a drain,
service endpoint and selector mismatches, and CoreDNS health.

Operational note, printed alongside any node finding: AKS node auto-repair reboots, then reimages,
then redeploys any node that stays NotReady for more than five minutes, emitting events from source
...
the 12 hours before the attempt. This targets 1.26 and later, excludes the read-only Get, List and
Watch verbs, and records usage hourly. It surfaces through the `Kubernetes API deprecations` detector
under the `Create, Upgrade, Delete and Scale` category.

Certificates: only a full control plane plus node pool upgrade renews expired cluster certificates. A
node image upgrade or a same-version node pool upgrade does not, so "upgrade the node image" is the
wrong advice for an expired certificate and fleetscan does not give it.
...
from `--timeout` and from SIGINT, so an interrupted run stops spawning work rather than orphaning
child processes. Per-context failures are collected, not propagated: the outer group returns an error
only for a fault that invalidates the whole run, such as cloudctx being absent or older than 1.4.0.
...
expired tenant is routine rather than exceptional. A run that aborted on the first one would be
useless in practice, and a run that silently omitted it would be worse than useless, because a clean
table would be read as a healthy fleet. The failure has to be visible in the output.

Throttling: honor `Retry-After`, then bounded exponential backoff. The per-subscription fan-in rule is
the primary defence against 429; retry logic is the fallback for what it does not cover, not the
strategy.
...
`doctor <cluster>` prints full detail for one cluster. `--category` narrows the detectors, `--since`
sets the window (default 24 hours, maximum 24 hours, and it must fall inside the last 30 days). The
same output formats apply. If the cluster name matches in more than one context, doctor exits 3 and
lists the matches; `--context` disambiguates.

```
$ fleetscan doctor aks-prod-01 --context acme-prod
cluster         aks-prod-01   westeurope
subscription    platform (00000000-0000-0000-0000-000000000000)
resource group  rg-platform-prod   node group MC_rg-platform-prod_aks-prod-01_westeurope
control plane   1.30.4   outdated, 1.30.7 offered in westeurope
tier            Standard, supportPlan KubernetesOfficial
state           provisioning Succeeded, power Running, resource health Available
channels        upgrade patch, node OS NodeImage
api server      private cluster, 2 authorized ranges, Azure RBAC enabled

node pools      NAME     COUNT  MIN/MAX  VERSION  STATE      SKEW
                system   3      3/5      1.30.4   Succeeded  ok
                user     15     3/30     1.29.8   Succeeded  ok

readiness       upgrade path     ok    1.30.4 -> 1.30.7
                surge quota      ok    33 of 100 vCPU used in westeurope
                subnet headroom  warn  41 free addresses, 15 needed at 33 percent surge
                MC_ group lock   ok    no lock
...
- The `detector` client is tested against an `httptest.Server` serving recorded response bodies,
  including a 429 with `Retry-After` and a malformed table. No live ARM call in any test.
- `classify()` and the SNAT and version-skew arithmetic are pure functions, table-driven tested, with
  no IO at all. The genuinely interesting logic here needs neither a cloud nor a network.
- `report` rendering is golden-file tested for table, JSON and CSV.
...
This project doubles as a Go learning exercise for its author, so the split is recorded rather than
left to chance.
